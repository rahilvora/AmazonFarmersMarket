    ...
    client.get("getProducts", function (err, result) {
        if (result !== null) {
            ...
            productsCollection.find({active: "Y", approved: "Y"}).toArray(function (err, data) {
                if (!err) {
                    console.log("Storing result in redis");
                    client.set("getProducts", JSON.stringify(data), function (err, ans) {
    ...
    client.get("getAddProductRequests", function (err, result) {
        if (result !== null) {
            ...
            productsCollection.find({active: "Y", approved: "N"}).toArray(function (err, data) {
                if (!err) {
                    console.log("Storing result in redis");
                    client.set("getAddProductRequests", JSON.stringify(data), function (err, ans) {
...
router.post('/upload', function (req, res) {
    upload(req, res, function (err) {
        console.log("here" + JSON.stringify(req.file));
        if (err) {
...
router.post('/createProduct', function (req, res, next) {
    ...
    console.log("in create prod" + req.body);
    ...
    var document = {
        ...
        active: 'Y',
        approved: 'N',
        productimage: req.body.filepath,
        productreviews: [],
        productrating: ""
    };
            ...
            if (results.length > 0) {
                ...
                connection.query(addFarmer, function (err, result) {
                    if (err) {
                        ...
                        farmerreviews.insert(document), function (err, user) {
                            if (user) {
...
router.post('/uploadFarmerImage', function (req, res) {
    upload(req, res, function (err) {
        console.log("here" + JSON.stringify(req.file));
        if (err) {
            res.json({error_code: 1, err_desc: err});
...
router.post('/uploadFarmerVideo', function (req, res) {
    upload(req, res, function (err) {
        console.log("here" + JSON.stringify(req.file));
        if (err) {
            res.json({error_code: 1, err_desc: err});
