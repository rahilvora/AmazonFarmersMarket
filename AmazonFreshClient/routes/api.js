...
router.put('/updateTruck',function(req,res){
    var query = "UPDATE `truckdetails` SET `truckid` = '"+req.body.truckid+"', " +
                "`truckinfo` = '"+req.body.truckinfo+"', `driverid = '"+req.body.driverid+"'` WHERE `truckdetails`.`truckid` = "+req.body.truckid+";";
    connection.query(query,function(err,result){
        if(err){
            throw err;
        }
